...
import { importTrainers } from './api';
import { resolveTeamOption } from '../../lib/teams';
import type { TeamOption } from '../../lib/teams';

interface ImportTrainersDialogProps {
...
function buildExampleCsv(teamOptions: TeamOption[]): string {
  ...
  const row1 = ['Peter', 'Beispiel', '04.03.1985', sample1?.categoryName ?? 'U15', sample1?.teamName ?? 'U15'];
  const row2 = ['Sandra', 'Muster', '17.11.1990', sample2?.categoryName ?? 'U15', sample2?.teamName ?? 'U15'];
  return toCsv([header, row1, row2]);
}

export function ImportTrainersDialog({ teamOptions, onClose, onImported }: ImportTrainersDialogProps) {
  ...
  function handleDownloadExample() {
    downloadTextFile('trainer-import-beispiel.csv', buildExampleCsv(teamOptions));
  }
  ...
  return (
    <Modal
      title="Trainer aus CSV importieren"
      onClose={onClose}
      footer={
        <>
          ...
          >
            {importing ? 'Importiere…' : `${validCount} Trainer importieren`}
          </Button>
        </>
      }
    >
